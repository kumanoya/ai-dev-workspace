...
経験則・学びの時系列メモは `docs/notes/` へ（`/save-note` で書き出し）。同ディレクトリは本方針の適用外で、運用は [docs/notes/README.md](docs/notes/README.md) が権威。
...
破壊的な `gh` コマンドは `.claude/settings.json` の `deny` でブロック済み（対象コマンドの列挙は同ファイルが権威）。破壊的操作を依頼された場合は意図と影響範囲を確認してから実行し、シークレット・環境変数の値の設定は `gh` 経由でなくユーザーに手動実行を促す。PAT が必要なコマンドで認証エラーが出た場合は [docs/setup/github-pat.md](docs/setup/github-pat.md) の参照をユーザーに促す。
