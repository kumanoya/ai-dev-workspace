...
視覚検証スキル（`figma-verify-screen` / `verify-design-diff`）から参照される共通手順。dev サーバーの確保から実装画面のスクリーンショット取得までを定める。スキル固有の差分（撮影サイズ・オプション）は各スキル側に記載がある。
実装スキル（`figma-implement-screen` / `fix-design-diff`）は本ファイルを Read しない — スクショはユーザーが視覚検証を明示的に依頼したときだけ撮る。
