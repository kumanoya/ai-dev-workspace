...
argument-hint: "[PRの目的の補足（省略可）] [#Issue番号 または チケットURL（省略可）] [--draft]"
---
...
1. pr-creator エージェント（Agent ツール、`subagent_type: pr-creator`）を起動し、以下を渡す:
   ...
   - `issue`: `$ARGUMENTS` に `#数字` またはチケット URL が含まれていればそれを渡す。**無ければ空で渡す**（文脈から Issue 番号を推測しない。無関係な Issue を自動クローズする事故を防ぐ）
